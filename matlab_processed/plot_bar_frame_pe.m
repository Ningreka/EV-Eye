...
for user_num = 1:48
    ...
    for session = 2:2
        ...
        for pattern = 1:1
            ...
            load([processedpath,'\Pixel_error_evaluation\frame\left\user',num2str(user_num),'_session_',num2str(session),'_0_',num2str(pattern),'.mat']);
...
for user_num = 1:48
    ...
    for session = 2:2
        ...
        for pattern = 1:1
            ...
            load([processedpath,'\Pixel_error_evaluation\frame\right\user',num2str(user_num),'_session_',num2str(session),'_0_',num2str(pattern),'.mat']);
...
right_pixel_error = [session_1_0_2_list;session_2_0_1_list;session_2_0_2_list];

right_pixel_error_each_user = mean(right_pixel_error,1);


pixel_error_each_user = (left_pixel_error_each_user + right_pixel_error_each_user)/2;

mean(pixel_error_each_user)

x = [1:48];
figure(1);
hold on;
b = bar(x,[pixel_error_each_user'],1.05);
