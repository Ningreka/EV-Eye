...
## Introduction EV-Eye
EV-Eye introduces the largest and most diverse multi-modal frame-event dataset for high frequency eyetracking in the literature. 
...
<!--感觉数据集的描述还是不太清楚，应该在前面把数据集详细介绍一下-->
<!--这个数据集是一个近眼高速眼动数据集，使用davis346和tobii同时采集，包含了event和frame的数据。包括4个session对应不同的眼动方式，对于48个用户，四个session中session_1_0_1和。。。对应saccade，。。。对应fixation-->
...
**Data_davis** includes near-eye grayscale images in **frames** folder and event streams in **events** folder captured by two sets of DAVIS346 event cameras for **left** and **right** eyes. Each user participates four sessions of data collection, the first two sessions capture both saccade and fixation states of the eye movement, the last two sessions record eye movement in smooth pursuit. 
